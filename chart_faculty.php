...
$mysqli = $conn;
	...
	for ($i=2014; $i<=$date ; $i++) { 
		$j=$i + 1;
		$sql1="SELECT * FROM online_course_organised WHERE Date_From>='$i-07-1' && Date_To<='$j-6-30' AND Fac_ID = (SELECT Fac_ID from facultydetails where Email = '$name')";
		$label = "$i" . "-" . "$j";
...
$result_start;
	for ($i=2014; $i<=$date ; $i++) { 
		$j=$i + 1;
		$sql1="SELECT * FROM online_course_attended WHERE Date_From>='$i-07-1' && Date_To<='$j-6-30'&& type_of_course='$types' AND Fac_ID = (SELECT Fac_ID from facultydetails where Email = '$name')";
		$label = "$i" . "-" . "$j";
		if($res = $mysqli->query($sql1)) {
			if($i == 2014){
				$result_start = $res->num_rows;
...
<a href="export_to_pdf.php" type="button" class="btn btn-success btn-sm" style="margin-left: 80%;"><span class="glyphicon ">Export to Pdf</span></a>
</div>
</div>
</form>
</div>
