...
if(isset($_POST['insert-image'])){
	...
	if(isset($_POST['applicable']))
	{
		...
		{
			$query = "Update online_course_organised set certificate_path ='NULL'  where OC_O_ID = $cardId";
             mysqli_query($conn,$query);
			 $success =1;
			$file_name = $cardId. "_organised.jpg";
			$file = "/opt/lampp/htdocs/extc/certificates/$file_name";
			if (!unlink($file)){
		...
		{
			$query = "Update online_course_organised set certificate_path ='not_applicable'  where OC_O_ID = $cardId";
             mysqli_query($conn,$query);
			 			 $success =1;
			$file_name = $cardId. "_organised.jpg";
			$file = "/opt/lampp/htdocs/extc/certificates/$file_name";
		...
		else if($_POST['applicable'] == 1)
		{
			if(isset($_FILES['image']))
			{
			  ...
			  $targetName="certificates/".$_POST['id']."_organised.jpg";  
			  ...
			  if(empty($errors)==true) {
				if (file_exists($targetName)) {   
				 ...
				 if($moved == true){
					 //successful
					 $query = "Update online_course_organised set certificate_path =' ".$targetName."'  where OC_O_ID = $cardId";
					 mysqli_query($conn,$query);
					 			 $success =1;
