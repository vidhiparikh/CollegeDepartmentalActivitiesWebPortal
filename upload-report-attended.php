...
if(isset($_POST['insert-image'])){
	...
	if(isset($_POST['applicable']))
	{
		...
		{
			$query = "Update online_course_attended set report_path ='NULL'  where OC_A_ID = $cardId";
             mysqli_query($conn,$query);
			 $success =1;
			$file_name = $cardId. "_attended.jpg";
			$file = "/opt/lampp/htdocs/extc/reports/$file_name";
			if (!unlink($file)){
		...
		{
			$query = "Update online_course_attended set report_path ='not_applicable'  where OC_A_ID = $cardId";
             mysqli_query($conn,$query);
			 			 $success =1;
			$file_name = $cardId. "_attended.jpg";
			$file = "/opt/lampp/htdocs/extc/reports/$file_name";
		...
		else if($_POST['applicable'] == 1)
		{
			if(isset($_FILES['image']))
			{
			  ...
			  $targetName="reports/".$_POST['id']."_attended.jpg";  
			  ...
			  if(empty($errors)==true) {
				if (file_exists($targetName)) {   
				 ...
				 if($moved == true){
					 //successful
					 $query = "Update online_course_attended set report_path =' ".$targetName."'  where OC_A_ID = $cardId";
					 mysqli_query($conn,$query);
					 			 $success =1;
